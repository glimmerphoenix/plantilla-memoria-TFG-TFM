...
BCOR=12mm, 				    % Set binding correction (8-12 mm for hardcover)
%BCOR=0mm,                  % Uncomment for no biding extra margin (symmetric layout)
DIV=12   			        % Good proportions for fontsize = 12
                            % Please, adjust DIV value if you change the document font size
                            % DIV=calc --> calculate optimal page layout;
                            % DIV=x --> manually set x to a value in [6,15]
...
\areaset[current]{\textwidth}{1.2954\textheight}
...
\setmainfont[]{Times New Roman}         % Main font from EIF thesis template guidelines
\setsansfont[Scale=0.9]{Lato}                     % Sans-serif font, alt [Scale=0.9]
\setmonofont[Scale=0.9]{Source Code Pro Medium}   % Monospaced font, alt [Scale=0.9]
% TODO: CHECK ERRORS WITH MATHSPEC AND SETTING CUSTOM MATH FONT
%% Math font
